/*
Computer animation assignment 2: Inverse Kinematics
 ...
 *
 */
void mainPanel(kinematics::Ball* ball, kinematics::Obstacle* obs);
/**
 ...
 * @param window: Current context
 */
void renderUI(GLFWwindow* window, kinematics::Ball* ball, kinematics::Obstacle* obs);
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        isStable = IK->inverseKinematics(ball.getCurrentPosition(), obs.getCurrentPosition(), obsActive, start_bone, end_bone);
        ball.setModelMatrix();
        obs.setModelMatrix();
...
void mainPanel(kinematics::Ball* ball, kinematics::Obstacle* obs) {
    ...
    if (ImGui::Begin("Control")) {
        ...
        ImGui::Checkbox("Activate Obstacle Cube", &obsActive);
        auto&& opos = obs->getCurrentPosition();
        Eigen::Vector4f obsPosition(opos[0], opos[1], opos[2], opos[3]);

        if (ImGui::SliderFloat("Obstacle x", &obsPosition[0], -10.0f, 10.0f)) {
            // ImGui::InputDouble("target x", &bpos[0], 0.01, 0.1, "%.2lf")
            // bpos[0] = std::clamp(bpos[0], -10.0, 10.0);
            opos[0] = obsPosition[0];
        }
        if (ImGui::SliderFloat("Obstacle y", &obsPosition[1], -10.0f, 10.0f)) {
            /*ImGui::InputDouble("target y", &bpos[1], 0.01, 0.1, "%.2lf")
            bpos[1] = std::clamp(bpos[1], -10.0, 10.0);*/
            opos[1] = obsPosition[1];
        }
        if (ImGui::SliderFloat("Obstacle z", &obsPosition[2], -10.0f, 10.0f)) {
            /*ImGui::InputDouble("target z", &bpos[2], 0.01, 0.1, "%.2lf")
            bpos[2] = std::clamp(bpos[2], -10.0, 10.0);*/
            opos[2] = obsPosition[2];
        }
    }
    ImGui::End();
}
